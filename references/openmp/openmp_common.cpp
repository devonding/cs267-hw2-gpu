...
void moveWithGrid(particle_t &p, subgrid *gridContainer) {
    ...
    if (id_old != id_new) {
        ...
        omp_set_lock(&gridContainer[id_old].lock);
        gridContainer[id_old].pArray.erase(&p);
        gridContainer[id_old].count --;
        omp_unset_lock(&gridContainer[id_old].lock);

        omp_set_lock(&gridContainer[id_new].lock);
        gridContainer[id_new].pArray.insert(&p);
        gridContainer[id_new].count ++;
        omp_unset_lock(&gridContainer[id_new].lock);
    }
    p.x = tmpx;
    p.y = tmpy;
}
